...
- **New `tab` widget — a sized slot with named tab panels, each holding its own widgets.** A layer dashboard widget can now be a `tab` container: a grid slot you size with span / row span, holding any number of **named tabs**, where each tab is its own little dashboard — its own set of widgets (card / line / top / table) in a sub-grid. Switching a tab swaps the whole set, and only the active tab's widgets are queried (lazy) — an unopened tab costs nothing until you open it, and a previously-viewed tab stays warm. Author it in the Layer dashboards admin: add a widget, set its type to `tab`, manage its tabs (add / rename / reorder / delete) from the drawer, then **Open** a tab to add and edit its widgets on the canvas — switch tabs to fill each one, and `+ Add widget` drops into the active tab. Drag an existing top-level widget onto a tab container to move it in; a tab can't nest a tab. Useful for packing related views — traffic / latency / Apdex, or one panel per subsystem — into one dense slot.
- **`+ Add widget` now lets you pick the kind.** The add button opens a small menu of widget kinds — card, line, top, record, table, or tab group — each with a one-line description, instead of always dropping in a card you then have to retype. Picking `tab group` seeds an empty named tab you fill by opening it.
