...
| **ui-management** | `SW_UI_MANAGEMENT=default` | 11.x | `/ui-management/templates*` | Dashboard templates — the layer / overview / alert / 3D template store the config surface reads and writes. Unreachable in live template mode blocks the dashboard-template admin surface; in readonly mode Horizon serves bundled templates and never calls it. |
