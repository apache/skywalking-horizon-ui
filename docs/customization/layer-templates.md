...
If OAP already holds a record stored under a name Horizon does not read — pushed by an older Horizon, by another tool, or by hand — the banner turns to **UNREADABLE** and lists each one with its OAP record id and the reason. Those records render for nobody and appear in no picker, which is exactly why the banner is the only place they can be seen. To clean one up, publish its content under the name the reason gives, then retire the old record on OAP; Horizon never retires a record on its own.
