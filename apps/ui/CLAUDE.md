...
**Features do not import from each other.** The one exception today is the admin editor for the 3D map, which reads that feature's own config composable — a page editing the thing it is the editor for, not one feature reaching into another's *data*, which is what the rule exists to stop.

A component that several features borrow is a primitive whichever folder it happens to sit in, and belongs in `components/primitives/`. `Modal` and `MonacoDiff` lived under `features/operate/_shared/` for a while because that is where they were first written; fifteen files across `shell/`, `render/`, `layer/` and three features imported them from there, which is the shape that tells you the address was wrong rather than the rule.
