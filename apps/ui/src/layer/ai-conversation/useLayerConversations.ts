...
export interface LayerConversationsParams {
  ...
  /** One sender, by instance name, or null for every sender of the runtime. */
  instanceName: Ref<string | null>;
  /** One conversation by id, or null. */
  conversation: Ref<string | null>;
  /** Text the title must contain, or null. OAP matches it. */
...
export function useLayerConversations(layerKey: Ref<string>, params: LayerConversationsParams) {
  const q = useQuery<AiConversationsResponse>({
    queryKey: [
      'layer-ai-conversations', layerKey, params.service, params.instanceName, params.conversation, params.title, params.windowMinutes,
    ],
    queryFn: ({ signal }) =>
      bffClient.aiConversation.list(
        layerKey.value,
        {
          ...
          ...(params.instanceName.value ? { instanceName: params.instanceName.value } : {}),
          ...(params.conversation.value ? { conversation: params.conversation.value } : {}),
          ...(params.title.value ? { title: params.title.value } : {}),
