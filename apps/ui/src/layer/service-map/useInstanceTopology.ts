...
import type { InstanceTopologyCall, InstanceTopologyNode, InstanceTopologyResponse } from '@skywalking-horizon-ui/api-client';
...
export function useInstanceTopology(
  ...
  const predicate = computed<GraphPredicate>(() => ({
    ...
    clientServiceId: clientServiceId.value,
  ...
  const q = useQuery({
    queryKey: ['layer-instance-topology', predicateKeyRef],
    queryFn: ({ signal }) =>
      fetchDrawable(() =>
  ...
  useRefreshErrorReport({ owner: 'Instance relationship', action: 'reading the instance relationship graph', error: q.error });
  ...
  useAutoRefreshSubscribe(
    ...
    () => ['layer-instance-topology', predicateKeyRef.value],
  );
  ...
  const { acceptedSnapshot, latestAttempt, phase, predicateGeneration } =
    useGraphState<InstanceTopologyResponse>({
      data: q.data,
      error: q.error,
  ...
  return {
    ...
    predicate,
    predicateKey: predicateKeyRef,
    nodes: computed<InstanceTopologyNode[]>(() => acceptedSnapshot.value?.nodes ?? []),
    calls: computed<InstanceTopologyCall[]>(() => acceptedSnapshot.value?.calls ?? []),
