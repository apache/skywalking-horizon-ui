...
export const auditPostgresSchema = z
  .object({
    ...
    /** Every audit query is background work — both writers, the sweep and the
     *  page query — because nothing runs on a request path. */
...
export const auditSchema = z
  .object({
     ...
     *  fires first flushes everything pending. */
    /** Capped at 500 because the store writes a batch in 500-row chunks with
     *  autocommit, and a failure in a later chunk re-sends the earlier ones
     *  that already committed. One batch, one chunk, no partial replay. */
    eventBatchRows: z.number().int().positive().max(500).default(50),
    eventBatchSeconds: z.number().int().positive().default(15),
...
export function parsePostgresUrl(raw: string): PostgresTarget | null {
  ...
  const host = (last('host') ?? url.hostname).replace(/^\[|\]$/g, '').toLowerCase();
