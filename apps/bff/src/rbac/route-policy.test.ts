...
describe('isTemplateWriteRoute — which routes the readonly backstop covers', () => {
  ...
  it('does NOT match reads, nor non-template writes (runtime-rule / local state stay editable)', () => {
    ...
    expect(isTemplateWriteRoute('POST', '/api/alarms/config')).toBe(false); // local state
