...
describe('POST /api/layer/:key/ai-conversations', () => {
  ...
  it('forwards the sender filter, a smaller limit, and the cold-stage flag', async () => {
    ...
    await app.inject({
      ...
      payload: { service: 'Claude Code', instanceName: 'me@host', conversation: ' c1 ', title: '  audit ', limit: 25 },
    });
    const sent = oap.asked('listConversations')[0]!.variables;
...
describe('GET /api/ai-conversation/:conversation/view', () => {
  ...
  const DOC = JSON.stringify({
    ...
    version: '1.0',
    conversation: 'c1',
    summary: { title: 'build }{ and check', state: 'verified', problems: [], talks: 3, steps: 21, streams: 2, segments: 1, rounds: 1, unresolved: 0 },
    talks: [],
  });
  ...
  it('holds the document, then relays it compressed with its decoded size and summary ahead of the bytes', async () => {
    ...
    // Only gzip is asked of OAP: the BFF decodes what it holds, and brotli or
    // zstd from OAP could not be counted.
    ...
    expect(res.headers['content-length']).toBe(String(DOC_GZIP.length));
    expect(res.headers['x-horizon-document-bytes']).toBe(String(Buffer.byteLength(DOC)));
    expect(JSON.parse(String(res.headers['x-horizon-document-summary']))).toEqual({ talks: 3, steps: 21, streams: 2, segments: 1, rounds: 1, unresolved: 0 });
    expect(res.rawPayload.equals(DOC_GZIP)).toBe(true);
  });

  it('asks OAP for plain bytes when the browser takes no gzip, and still states the size', async () => {
    let seenEncoding: string | undefined;
    const url = await serveOap((req, res) => {
      seenEncoding = req.headers['accept-encoding'];
    ...
    expect(res.headers['content-length']).toBe(String(Buffer.byteLength(DOC)));
    expect(res.headers['x-horizon-document-bytes']).toBe(String(Buffer.byteLength(DOC)));
    expect(res.body).toBe(DOC);
  });

  it('answers 502 when OAP sends gzip that does not decode, rather than relaying it blind', async () => {
    ...
    const { app, cookie: c } = await build(fakeOap().fetch, { oap: { queryUrl: url } });
    const res = await app.inject({ method: 'GET', url: '/api/ai-conversation/c1/view?service=x', headers: { cookie: c, 'accept-encoding': 'gzip' } });
    expect(res.statusCode).toBe(502);
    expect(res.json().message).toContain('could not be decoded');
  });
