...
const EVENT_COLUMNS =
  'at,kind,provider,outcome,reason,username,mail,roles,client_ip,horizon_ip,horizon_node';
...
describe('writing sign-in rows', () => {
  it('inserts a multi-row batch', async () => {
    const rows = [
      [new Date('2026-08-22T14:30:00Z'), 'local', null, 1, null, 'alice', null, 'admin', '203.0.113.7', null, 'pod-1:aaa'],
      [new Date('2026-08-22T14:31:00Z'), 'sso', 'github', 0, 'no_roles', 'bob@x.io', 'bob@x.io', null, null, null, 'pod-1:aaa'],
    ];
    await db.query(
      `INSERT INTO horizon_audit (${EVENT_COLUMNS}) VALUES ${valuesClause(2, 11)}`,
      rows.flat(),
    );
...
describe('writing token aggregates', () => {
   ...
   */
  const upsert =
    `INSERT INTO horizon_audit (${AGGREGATE_COLUMNS}) VALUES ${valuesClause(1, 8)}` +
