...
import { overFetchSize, takeOverFetched, type PageResult } from '../../../logic/paging/read-page.js';
 ...
 *  parameters; at 16 columns that is ~4000, and 500 keeps every statement
 *  small and the arithmetic obvious. */
...
const EVENT_COLUMNS = [
  'at', 'kind', 'provider', 'outcome', 'reason', 'username', 'mail', 'roles',
...
const SELECT_COLUMNS =
  'id, at, kind, provider, outcome, reason, username, mail, roles, ' +
...
export class PostgresAuditStore implements AuditStore {
  ...
  async open(): Promise<void> {
    ...
    try {
      ...
      if (!target || target.host === '') throw new AuditStoreError('schema_error');
      pool = new pg.Pool({
        host: target.host,
        port: target.port,
        user: target.user,
  ...
  async probe(): Promise<{ available: boolean; error?: StoreError }> {
    try {
      ...
      // index. Columns can all be present while that index is missing, and
      // then every probe passes and every upsert fails with 42P10 — the exact
      // drift `autoMigrate: false` allows.
      const idx = await this.db.query<{ n: number }>(
        `SELECT count(*)::int AS n FROM pg_indexes
          WHERE tablename = 'horizon_audit' AND indexname = 'horizon_audit_bucket_idx'`,
      );
      if ((idx.rows[0]?.n ?? 0) === 0) return { available: false, error: 'schema_error' };
  ...
  async writeEvents(rows: ReadonlyArray<AuditEvent & StoreStamp>): Promise<void> {
    ...
    for (let i = 0; i < rows.length; i += CHUNK_ROWS) {
      ...
      const params = chunk.flatMap((r) => [
        new Date(r.at), r.kind, r.provider ?? null, r.outcome, r.reason ?? null,
  ...
  async query(filter: AuditFilter): Promise<PageResult<AuditEntry>> {
    ...
    const offset = (Math.max(1, filter.pageNum) - 1) * size;
