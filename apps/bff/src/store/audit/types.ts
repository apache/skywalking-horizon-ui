...
export interface AuditFields {
  ...
  principalKey?: string;
  /**
   * Which SSO protocol proved this identity. NOT stored — the row says `sso`
   * and the provider. It selects between the two sign-on series the chart
   * draws, because verifying a signed ID token and reading an address from a
   * userinfo call are different assurances.
   */
 ...
 * best-effort per node, and the sign-in table is the exact record.
 */
...
export interface AuditService {
   ...
   * comes first, and a crash loses at most that window.
   */
  ...
  query(filter: AuditFilter): Promise<PageResult<AuditEntry>>;
