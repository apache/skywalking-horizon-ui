...
/** CI entry point for the bundled-template validator (`pnpm
 *  templates:validate`, chained into the repo-root `pnpm lint`). Exits
...
if (findings.length === 0) {
  ...
  console.error(`\ntemplates:validate: ${findings.length} finding(s)`);
  process.exit(1);
}
