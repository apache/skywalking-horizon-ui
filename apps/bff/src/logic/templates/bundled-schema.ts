...
const instanceTopologySchema = z
  .object({
    nodeMetrics: z.array(topologyMetricSchema).min(1),
...
const processTopologySchema = z
  .object({
    edgeClientMetrics: z.array(topologyMetricSchema).min(1),
    edgeServerMetrics: z.array(topologyMetricSchema).min(1),
  })
  .strict();
      ...
      .array(
        z
          .object({
            from: z.string().min(1),
            to: z.string().min(1),
            primary: z.union([z.string().min(1), z.array(z.string().min(1))]).optional(),
            metrics: z.array(deploymentMetricSchema).min(1),
          })
          .strict(),
      )
      .optional(),
 ...
 *  synthesises; layer headers never carry it). */
...
const aliasesSchema = z
  .object({
    ...
    instances: z.string().min(1).optional(),
    endpoints: z.string().min(1).optional(),
    endpointDependency: z.string().min(1).optional(),
    topology: z.string().min(1).optional(),
    instanceTopology: z.string().min(1).optional(),
...
export const layerTemplateSchema = z
  .object({
    ...
    documentLink: z.string().min(1).optional(),
    ...
    naming: z
      ...
      .strict()
      .optional(),
    instances: z.object({ badge: z.string().min(1).optional() }).strict().optional(),
  })
  .strict();
