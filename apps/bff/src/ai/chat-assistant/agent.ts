...
/**
 * The agent runner. Wires the shared tool registry onto a LangGraph
...
export async function runAiChat(args: RunAiChatArgs): Promise<void> {
  ...
  // The `prompt` (resolved system text) + the tool schemas from buildTools are
  ...
  const agent = createReactAgent({ llm: model, tools: buildTools(ctx), prompt: args.systemPrompt });
