...
import { createGraphicCardBuffer } from './graphic-card-buffer.js';
import type { ChatFigure, PodLogSpec, GraphicCard } from './graphic-card.js';
...
type FigureEvent = Extract<GraphicCard, { type: 'figure' }>;
function harness() {
  const events: FigureEvent[] = [];
  const buf = createGraphicCardBuffer((ev) => {
    if (ev.type === 'figure') events.push(ev);
  });
  return { ...buf, events };
}

describe('createGraphicCardBuffer', () => {
  ...
  it('emitPodLogs shares the figure number and flushes a pending group first', () => {
    const events: GraphicCard[] = [];
    const buf = createGraphicCardBuffer((ev) => events.push(ev));
