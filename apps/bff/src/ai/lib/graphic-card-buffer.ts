...
export interface GraphicCardBuffer {
...
export function createGraphicCardBuffer(send: (block: GraphicCard) => void): GraphicCardBuffer {
  let figureN = 0;
  ...
  const emitFigure: GraphicCardBuffer['emitFigure'] = ({ title, figures, group }) => {
    if (group) {
