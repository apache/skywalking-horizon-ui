 ...
 * `ai/lib/skills/rca/<id>.md` (frontmatter + body) — the prose lives as
 * content, not TS source; this module just loads and orders them.
 */

import { readResource } from '../../skills/loader.js';
...
function loadPlaybook(id: string): Playbook {
  const raw = readResource(`rca/${id}.md`);
  const m = /^---\r?\n([\s\S]*?)\r?\n---\r?\n?([\s\S]*)$/.exec(raw);
  if (!m) throw new Error(`Malformed playbook resource: rca/${id}.md (missing frontmatter)`);
