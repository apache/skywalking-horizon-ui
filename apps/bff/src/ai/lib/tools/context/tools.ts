...
export function contextTools(ctx: ToolContext): StructuredToolInterface[] {
  ...
  // Orientation for "can Horizon read anything right now?". Exists because an
  // unreadable template store and an idle deployment both look like empty
  // results, and only this tool can tell them apart (see
  // `metric-catalog/unreadable.ts`). Reuses the same probe `/api/preflight`
  ...
  const health = tool(
    async (): Promise<string> => {
      ...
      // `reachable: null` means NOT PROBED (ui-management is skipped in
      // templates.mode=readonly), which is not a failure — every other reader
      // treats null as N/A, and counting it would call a healthy readonly
      // deployment unhealthy.
      const unreachable = pre.modules.filter((m) => m.reachable === false).map((m) => m.name);
      return JSON.stringify({
        adminUrl: pre.adminUrl,
        adminReachable: pre.adminReachable,
        adminError: pre.adminError,
        templatesMode: pre.templatesMode,
        unreachableModules: unreachable,
        healthy: pre.adminReachable && unreachable.length === 0,
        note: pre.adminReachable
          ? unreachable.length === 0
            ? 'Horizon can read OAP. An empty catalog means the layer genuinely has no metrics at that scope.'
            : 'OAP admin is reachable but some modules are not. Features backed by them read empty; name the module when reporting.'
          : "OAP's admin endpoint is unreachable, so NO layer's templates or metrics can be read. Report this as an OAP health problem and stop — retrying other layers will not help.",
      });
    },
    {
      name: 'check_horizon_health',
      description: toolPrompt('context', 'check_horizon_health').description,
      schema: z.object({}),
    },
  );

  return [listLayers, listServices, health];
}
