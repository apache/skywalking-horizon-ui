...
- topology (`show_topology`) — the graph around the NAMED service(s): one seed or several, walked `depth` hops (2 by default). Nodes carry role metrics (center = LOAD number, ring = HEALTH band per the template's thresholds), edges carry server/client metrics.
- layer topology (`show_layer_topology`) — the SAME graph with no seed: every service in the layer and every call between them, one hop. A separate capability from the one above rather than an optional argument, because they answer different questions and walking a layer service-by-service repeats edges.
...
- endpoint-dependency (`show_endpoint_dependency`) — a NAMED endpoint's up/down dependency chain, or the busiest one when none is named.
...
**A terminal client gets DATA, not a drawing.** Horizon renders for one surface only — the web UI, and the `ui://` card bundle that reuses it in a host able to mount a frame. Nothing draws for a terminal: no Unicode renderer, no render tool. An agent with no frame reads the capture and formats it itself, and `presentation.terminal.md` says how per kind — a time series becomes prose, top-N and tables become markdown tables, every map becomes a nodes table plus a calls table, profiling becomes a ranked list of hot frames plus a pointer to the profiling tab. Adding a card kind therefore means: the Vue block, a note in `cards.yaml` telling a model how to read that payload, and the summary in `skills.md`.
