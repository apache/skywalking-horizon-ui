...
- **Layering — UI.** `apps/ui/src/` follows the same role-based grouping. The guiding rule is **feature code stays cohesive with its feature; shared code is feature-AGNOSTIC only** (fonts, styles, formatters, generic primitives, charts wrappers). A component or composable that knows about a feature's data lives WITH the feature, not in a shared pile. Features don't import from each other.
  ...
  - **`packages/conversation-view/`** — the one renderer that lives outside `apps/ui`: a framework-free module that draws an `asz.view` document (an AI agent conversation) on the `--sw-*` tokens. Horizon wraps it in a Vue component; the AI Sessionizer builds the same package from a pinned Horizon commit and embeds it, so it must stay free of Vue, of `fetch`, and of any colour that is not a token. Its host-shell output (tokens + themes + fonts) exists for that other host; Horizon never imports it.
