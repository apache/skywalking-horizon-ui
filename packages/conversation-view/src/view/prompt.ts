...
export function drawPrompt(ctx: ViewContext, body: HTMLElement, e: Step): void {
  ...
  // A file the server was asked for and did not answer is not offered again, since asking brings the
  // same answer; the panel draws the bodies that did rebuild over a note naming what is not there.
  // A read that ended in an error is offered again whatever it covered, with what went wrong.
  const unasked = outcome?.failed ? absent : absent.filter((seq) => !(outcome?.missing ?? []).includes(seq));
  if (unasked.length) {
