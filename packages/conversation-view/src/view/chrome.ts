...
export function drawStatus(ctx: ViewContext): void {
  ...
  ctx.q('.acv-problems-body').innerHTML = sum.problems.length
    ? `<ul>${sum.problems.map((p) => `<li>${esc(p)}</li>`).join('')}</ul>`
    : '';
  // A document that folded nothing has only its problems to show: the box
  // starts open, so the reader is not left with an empty transcript and a
  // badge to find.
  setProblemsOpen(ctx, sum.steps === 0 && sum.problems.length > 0);
}

/** The problems float under the strip, as the overview does; the two never
 *  show together. */
export function setProblemsOpen(ctx: ViewContext, open: boolean): void {
...
export function setOverviewOpen(ctx: ViewContext, open: boolean): void {
...
export function drawTalkList(ctx: ViewContext): void {
  ...
  // A row is a whole talk: what came in, and what the agent answered.
