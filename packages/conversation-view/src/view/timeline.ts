...
function layout(ctx: ViewContext, stream: string): Layout {
  ...
  const lanes: Lane[] = [];
  // Room for the time stamps floating above; no stream heading row. The dock's
  // own heading names the stream, and the lanes are the whole of it.
  let y = 24;
  const streamY = y;
...
export function drawTimeline(ctx: ViewContext): void {
  ...
  // A step without a time (the runtime recorded none) is placed at the time
  // of the step before it in reading order, or the first known one: it stays
  // on the axis where it was read, instead of at a time that does not exist.
  ...
  const firstKnown = m.flow(stream).find((e) => e.at)?.at ?? 0;
  const at = (e: Step): number => e.at || known.get(e.id) || firstKnown;
