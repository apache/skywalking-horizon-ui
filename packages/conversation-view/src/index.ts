...
export function mountConversationView(host: HTMLElement, opts: MountOptions): ConversationView {
  ...
  function switchStream(name: string): void {
    ...
    // parent's counts.
    const talkId = model.step(state.sel)?.talk ?? model.talksOf(name)[0]?.id ?? null;
  ...
  function centerOn(id: string | null, behavior: ScrollBehavior, alsoTranscript = true): void {
    ...
    const card = list.querySelector<HTMLElement>(`[data-card="${cssEscape(id)}"], [data-step="${cssEscape(id)}"]`);
    if (card) {
  ...
  // The floating overview closes on a click anywhere else, as a menu does.
