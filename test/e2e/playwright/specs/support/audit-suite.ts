...
/**
 * The login audit log, end to end — as a suite two cases share.
 *
 * The feature is one feature: a sign-in becomes a row an operator can read,
 * and a token use becomes a count beside it. Which database holds that is a
 * deployment choice, so the same assertions run against each rather than each
 * getting its own approximation of them. A behaviour that differs between the
 * two is a defect in one of them, and this is what would show it.
 */

import { test, expect, request as playwrightRequest } from '@playwright/test';
import { E2E_USER, E2E_TOKEN, E2E_TOKEN_ID, E2E_TOKEN_USER } from '../fixture.js';
...
export function auditSuite(storage: string): void {
    test.describe(`login audit (${storage})`, () => {
    ...
    test('counts token use on its own tab, and never as a sign-in', async ({ page, baseURL }) => {
      ...
      // It is NOT a sign-in, and this is the assertion that says so: a token
      // recorded as one would appear on the Login tab under the token's OWN
      // identity, which nothing else in the suite ever signs in as.
      await expect(page.locator('tbody tr', { hasText: E2E_TOKEN_USER })).toHaveCount(0);
      await expect(page.locator('tbody tr', { hasText: E2E_TOKEN_ID })).toHaveCount(0);

      // The selector is not merely mis-spelled: the same one DOES match the
      // account that really signed in, so a zero above is an absence rather
      // than a typo.
      await expect(page.locator('tbody tr', { hasText: E2E_USER })).not.toHaveCount(0);
