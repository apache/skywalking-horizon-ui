...
- **depending on which entity a page opens on** — a layer ranks its services by a metric, and the basis for that ranking is not constant: the header reads a completed HOUR, so it changes at ten past every hour, and on a fixture young enough to have no hour yet it falls back to the picked window. A spec that reads one service's data and relies on that service happening to be selected therefore passes or fails on when the run started. Only the fixture's log traffic reaches the provider, so a Logs spec needs it by name — `selectService(page, PROVIDER_SERVICE)` — not by luck. Asserting that the layer opens on a sensible service is a fine test; it is just a DIFFERENT test, and it already exists.
